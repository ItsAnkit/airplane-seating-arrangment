...
describe SeatingArrangement, type: :class do
  ...
  context 'when provided with queue size and arrangement pattern' do
    ...
    it 'will not fill window and central seats if queue size is less' do
      ...
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][0]).eql?(' ')
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][1]).eql?(' ')
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][2]).eql?(1)
    end
    ...
    it 'will not fill central seats if queue size is not sufficient' do
      ...
      airplane = SeatingArrangement.new(@structure, passengers_count)
      airplane.print_arrangement
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][0]).eql?(24)
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][1]).eql?(' ')
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][2]).eql?(1)
    end
    ...
    it 'will fill all types of seats' do
      ...
      airplane.print_arrangement
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][0]).eql?(24)
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][1]).eql?(31)
      expect(airplane.instance_variable_get(:@compartment_arrangement)[0][0][2]).eql?(1)
    end
    ...
    #   airplane = SeatingArrangement.new(@structure, passengers_count)
    #   airplane.print_arrangement
    #   expect(airplane.instance_variable_get(:@message)).eql?('Airline can arrange for maximum 36 seats.')
    # end
  end
end
